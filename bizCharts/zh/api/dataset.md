...
存储已注册的 Connector（key-value 对），Connector 用法见 [Connector](./connector)。

### transforms

存储已注册的 Transform（key-value 对），Transform 用法见 [Transform](./transform)。

## 静态方法

### registerConnector

`DataSet.registerConnector(name, connector)` 注册一个数据连接函数，注册后所有数据视图都可以使用 `name` 来引用这个数据连接函数，从而接入某种数据源。
...
*  `connector`
    * Function
    * 描述：connector 的参数是原始数据 `data` 和配置项 `options`，需要返回 `DataSet.View` 实例所需要的数据。

        - `data`

            源数据，DataSet.View 实例 `source` 方法的第一个参数。

        - `options`

            DataSet.View 实例 `source` 数据时指定的配置项，即第二个参数，是一个可选值。

举个简单例子，如果要注册一个接入 CSV 数据的 Connector，则实现如下：
...
`DataSet.registerTransform(name, transform)` 注册一个数据处理函数，注册后所有数据视图都可以使用 `name` 来引用这个数据处理函数，从而进行某种数据处理。
...
- `transform`       **Function**
    ...
    对数据进行变换处理的函数，参数分别是 DataSet.View 实例 `dv` 和调用 transform 时指定的配置项 `options`。

    - `dv`      **object**

        数据视图，`DataSet.View` 的实例。

    - `options`     **object**

        调用数据视图的 `transform` 方法来进行转换时指定的配置项。

举个简单例子，如果要注册一个过滤数据的 Transform，则实现如下：
