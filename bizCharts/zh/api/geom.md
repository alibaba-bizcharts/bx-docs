...
type | 说明 | 备注
--- | --- | ---
`point` | 点，用于点图的构建。 | 点的形状有很多，也可以使用图片代表点（气泡图），同时点也可以在不同坐标系下显示，所以可以扩展出非常多的图表类型。
`path` | 路径图，地图上的路径 | 路径图是无序的线图。
`line` | 线，点按照 x 轴连接成一条线，构成线图。 | 在极坐标系下可以转换成雷达图。
`area` | 填充线图跟坐标系之间构成区域图，也可以指定上下范围。 | 极坐标系下可用于绘制雷达区域图
`interval` | 使用矩形或者弧形，用面积来表示大小关系的图形，一般构成柱状图、饼图等图表。 | 通过坐标系的转置、变化，可以生成各种常见的图表类型；所有的图表都可以进行层叠、分组。
`polygon` | 多边形，可以用于构建热力图、地图等图表类型。 | 多个点可以构成多边形。
